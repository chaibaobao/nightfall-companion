...
export default function App() {
  ...
  useEffect(() => { mounted.current = true; return () => { mounted.current = false; flowId.current += 1; stopAllAudio() } }, [])
  ...
  const beginGame = async () => {
    ...
    const run = ++flowId.current
    stopAllAudio(); setBusy(true)
    setBlackCatRevealed(false)
    setGame({ ...initialGame, players: draftPlayers.map((p) => ({ ...p, name: p.name.trim() })), phase: 'DAWN_PHASE' })
    await startBackgroundMusic(voice)
    if (run !== flowId.current) return
    await say(['dawn-intro'])
    if (run !== flowId.current) return
  ...
  const selectPlayer = async (id: number) => {
    ...
    if (game.phase === 'WITCH_BLACK_CAT_ACTION') {
      ...
      await say(['dawn-complete'])
      if (run !== flowId.current) return
      stopBackgroundMusic()
      changePhase('DAY_START')
    } else if (game.phase === 'WITCH_KILL_ACTION') {
      setGame((current) => ({ ...current, witchKillTarget: id, phase: 'NIGHT_PHASE' }))
      await say([game.sheriffEnabled ? 'witch-complete-sheriff' : 'witch-complete-no-sheriff'])
      if (run !== flowId.current) return
      if (game.sheriffEnabled) changePhase('SHERIFF_PROTECT_ACTION')
      else { stopBackgroundMusic(); finishNight(id, null) }
    } else if (game.phase === 'SHERIFF_PROTECT_ACTION') {
      setGame((current) => ({ ...current, sheriffProtectTarget: id, phase: 'NIGHT_PHASE' }))
      await say(['sheriff-complete'])
      if (run !== flowId.current) return
      stopBackgroundMusic()
  ...
  const endGame = () => { flowId.current += 1; stopAllAudio(); setBusy(false); changePhase('GAME_COMPLETE') }
  const reset = () => { flowId.current += 1; stopAllAudio(); clearGame(); setGame(initialGame); setDraftPlayers(makePlayers(6, [])); setBlackCatRevealed(false); setBusy(false); setSettingsOpen(false) }
  const replayPrompt = async () => {
    ...
    if (ids[game.phase]) await say(ids[game.phase]!)
  }
