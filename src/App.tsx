...
import { getAudioAssetNames, NARRATION_LINES, pause, removeAudioAsset, saveAudioAsset, speakLines, stopSpeech, withBackgroundMusic } from './speech'
...
export default function App() {
  ...
  const beginGame = async () => {
    ...
    await scene(async () => { await say(['dawn-start', 'all-close-eyes']); await pause(3000); await say(['witch-open-eyes', 'black-cat-select', 'witch-any-player']) })
  ...
  const beginNight = async () => {
    ...
    await scene(async () => { await say(['night-falls', 'all-close-eyes']); await pause(3000); await say(['witch-open-eyes', 'witch-kill-select', 'witch-any-player']) })
  ...
  const selectPlayer = async (id: number) => {
    ...
    if (game.phase === 'WITCH_BLACK_CAT_ACTION') {
      ...
      await scene(async () => { await say(['black-cat-placed', 'witch-close-eyes']); await pause(3000); await say(['day-breaks']) })
      changePhase('DAY_START')
    } else if (game.phase === 'WITCH_KILL_ACTION') {
      setGame((current) => ({ ...current, witchKillTarget: id, phase: 'NIGHT_PHASE' }))
      await scene(async () => { await say(['target-selected', 'witch-close-eyes']); await pause(3000); if (game.sheriffEnabled) await say(['sheriff-open-eyes', 'sheriff-select', 'sheriff-rule']); else await say(['day-breaks']) })
      ...
      await scene(async () => { await say(['target-selected', 'sheriff-close-eyes']); await pause(3000); await say(['day-breaks']) })
      finishNight(game.witchKillTarget!, id)
    }
    if (mounted.current) setBusy(false)
  }

  const revealResults = async () => { setBusy(true); await scene(() => say(['results-intro'])); changePhase('RESULTS_REVEALED'); if (mounted.current) setBusy(false) }
  const reset = () => { stopSpeech(); clearGame(); setGame(initialGame); setDraftPlayers(makePlayers(6, [])); setBusy(false); setSettingsOpen(false) }
  const replayPrompt = async () => {
    const ids: Partial<Record<Phase, string[]>> = { WITCH_BLACK_CAT_ACTION: ['black-cat-select', 'witch-any-player'], WITCH_KILL_ACTION: ['witch-kill-select', 'witch-any-player'], SHERIFF_PROTECT_ACTION: ['sheriff-select', 'sheriff-rule'] }
  ...
  return <main className="app-shell">
    ...
    <section className={`stage ${game.phase === 'START' ? 'stage-setup' : ''}`} key={game.phase}>
      ...
      {game.phase === 'DAY_START' && nightChoice}
